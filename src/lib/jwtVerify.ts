import { createRemoteJWKSet, jwtVerify, decodeProtectedHeader, decodeJwt, exportSPKI, type JWTPayload } from 'jose';
...
export async function verifyJwtSignature(token: string, jwksUrl: string, expectedKid?: string): Promise<VerifyResult> {
  try {
    ...
    const payload = decodeJwt(token) as JWTPayload & { tid?: string; ver?: string };
    ...
    const { protectedHeader, payload: verifiedPayload } = await jwtVerify(token, JWKS, {
    ...
    try {
      ...
      const isKeyNotFound = errorMsg.includes('kid') || errorMsg.includes('key') || errorMsg.includes('JWK');
      
      return {
        ok: false,
        keyFound: !isKeyNotFound,
        error: errorMsg,
        reason: isKeyNotFound ? 'Key not found or signature mismatch' : 'Verification failed',
        alg: header.alg,
        kid: header.kid,
        ver: payload.ver,
        iss: payload.iss,
