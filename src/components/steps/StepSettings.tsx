...
export default function AuthCodeStepSettingsCommon(props: Props) {
  ...
  return (
    <>
      ...
      <div className="mt-5">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginLeft: '0.5rem' }}>
            ...
            />
            <Tooltip target={`#${localStorageIconId}`} content={maybeT('sections.settings.localStorageNotice', "These values will be saved in your browser's local storage for convenience.")}
              style={{ fontSize: '0.85rem' }} />
          </div>
        </div>

        <p className="mb-3 mt-3 text-sm opacity-75">{maybeT('sections.settings.userProvidedDescription', 'These fields are configurable by you and are used to build the authorization request.')}</p>
        ...
        <div className="grid formgrid p-fluid gap-3">
          <div className="col-12 md:col-12">
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
              ...
              <div>
                <InputText id="tenantId" value={tenantId} onChange={(e) => setTenantId(e.target.value)} placeholder={t('placeholders.tenantId')} className={!tenantIdValid ? 'p-invalid' : ''} style={{ width: '100%' }} />
                {!tenantIdValid && (
          ...
          <div className="col-12 md:col-12">
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
              ...
              <div>
                <InputText id="clientId" value={clientId} onChange={(e) => setClientId(e.target.value)} placeholder={t('placeholders.clientId')} className={!clientIdValid ? 'p-invalid' : ''} style={{ width: '100%' }} />
                {!clientIdValid && (
          ...
          <div className="col-12 md:col-12">
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
              ...
              <div>
                <InputText id="scopes" value={scopes} onChange={(e) => setScopes(e.target.value)} placeholder={t('placeholders.scopes')} style={{ width: '100%' }} />
              </div>
            </div>
          </div>
          ...
          <div className="col-12 md:col-12">
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
              <div style={{ textAlign: 'left' }}>
                <LabelWithHelp id="streamlined" text={t('labels.streamlined')} help={t('help.streamlined')} />
              </div>
              <div>
                <div className="flex align-items-center gap-2">
                  <InputSwitch inputId="streamlined" checked={!!streamlined} onChange={(e) => setStreamlined(!!e.value)} />
                  <label htmlFor="streamlined" className="m-0">{streamlined ? t('toggles.streamlinedOn') : t('toggles.streamlinedOff')}</label>
                </div>
              </div>
            </div>
          </div>
          ...
          {showPkceToggle && (
            <div className="col-12 md:col-12">
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
                <div style={{ textAlign: 'left' }}>
                  <LabelWithHelp id="pkceEnabled" text={t('labels.pkceEnabled')} help={t('help.pkceEnabled')} />
                </div>
                <div>
                  <div className="flex align-items-center gap-2">
                    <InputSwitch inputId="pkceEnabled" checked={!!pkceEnabled} onChange={(e) => setPkceEnabled(!!e.value)} />
                    <label htmlFor="pkceEnabled" className="m-0">{pkceEnabled ? t('toggles.pkceEnabledOn') : t('toggles.pkceEnabledOff')}</label>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="mt-5">
        <h5 className="mb-2">{maybeT('sections.settings.resolvedTitle', 'Resolved (read-only) values')}</h5>
        <p className="mb-3 mt-3 text-sm opacity-75">{maybeT('sections.settings.resolvedDescription', 'These values are derived from the tenant and other inputs. They are shown for reference and cannot be changed here.')}</p>
        ...
        <div className="grid formgrid p-fluid gap-3">
          {showAuthEndpoint && (
            <div className="col-12 md:col-12">
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
                ...
                <div>
                  <InputText id="authEndpoint" value={resolvedAuthEndpoint} readOnly style={{ width: '100%' }} />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="col-12 md:col-12">
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
              ...
              <div>
                <InputText id="tokenEndpoint" value={resolvedTokenEndpoint} readOnly style={{ width: '100%' }} />
              </div>
            </div>
          </div>
          ...
          {showRedirectUri && (
            <div className="col-12 md:col-12">
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(15rem, 20rem) 1fr', alignItems: 'center', columnGap: '1rem' }}>
                ...
                <div>
                  <InputText id="redirectUri" value={redirectUri} readOnly className={!redirectUriValid ? 'p-invalid' : ''} style={{ width: '100%' }} />
                  {!redirectUriValid && (
