...
const ACCESS_CLAIMS_DOC = 'https://learn.microsoft.com/en-us/entra/identity-platform/access-token-claims-reference';
const ID_CLAIMS_DOC = 'https://learn.microsoft.com/en-us/entra/identity-platform/id-token-claims-reference';
const OPTIONAL_CLAIMS_DOC = 'https://learn.microsoft.com/en-us/entra/identity-platform/optional-claims-reference';
...
export default function StepDecode({ accessToken, idToken, decodedAccessHeader, decodedAccessPayload, decodedIdHeader, decodedIdPayload, onDecodeTokens }: Props) {
  ...
  const [activeDialogToken, setActiveDialogToken] = useState<TokenType | null>(null);
  const claimDescriptions = (t.raw('claimDescriptions') as Record<string, string>) ?? {};
  ...
  const accessClaims = useMemo(
    () => parsePayloadToClaims(decodedAccessPayload, claimDescriptions, unknownClaimDescription),
    [decodedAccessPayload, claimDescriptions, unknownClaimDescription],
  );
  const idClaims = useMemo(
    () => parsePayloadToClaims(decodedIdPayload, claimDescriptions, unknownClaimDescription),
  ...
  const activeDialogTitle =
    activeDialogToken === 'access' ? t('claimsDialog.accessTitle') : t('claimsDialog.idTitle');
  const primaryDocUrl = activeDialogToken === 'access' ? ACCESS_CLAIMS_DOC : ID_CLAIMS_DOC;
            ...
            <div className="w-full" style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', alignItems: 'start' }}>
              ...
              <div>
                <div className="flex align-items-center gap-2 mb-2">
                  ...
                  >
                    <i className="pi pi-file" style={{ fontSize: '1rem' }} aria-hidden="true" />
                  </button>
                </div>
                <InputTextarea
                  id="accessPayload"
        ...
        {!activeClaims.length ? (
          ...
          <div className="overflow-auto">
            <table className="w-full" style={{ tableLayout: 'fixed' }}>
              ...
              <thead>
                <tr>
                  <th className="text-left p-2">{t('claimsDialog.columns.name')}</th>
                  <th className="text-left p-2">{t('claimsDialog.columns.value')}</th>
                  <th className="text-left p-2">{t('claimsDialog.columns.description')}</th>
                </tr>
              </thead>
              <tbody>
                {activeClaims.map((claim) => (
                  <tr key={claim.name} className="surface-border border-bottom-1">
                    <td className="p-2" style={{ verticalAlign: 'top', wordBreak: 'break-word' }}>{claim.name}</td>
                    <td className="p-2" style={{ verticalAlign: 'top', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{claim.value}</td>
                    <td className="p-2" style={{ verticalAlign: 'top', wordBreak: 'break-word' }}>{claim.description}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="mt-3">
              <p className="m-0 mb-2 font-semibold">{t('claimsDialog.references.title')}</p>
